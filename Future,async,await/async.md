...
1. First In First Out(FIFO)의 선입선출 방식으로 "`MicroTask`와 Event" 준비
    - 이벤트 큐로 넘어가기 전에 아주 짧은 시간동안 `비동기적`으로 먼저 실행되고 끝나는 작은 작업을 의미
        - 큐(Queue)의 사전적의미로는 `줄, 혹은 줄을 서서 기다리는 것`이며 `선입선출(FIFO)`방식의 자료구조이다.
          삭제연산만 하는 곳을 프론트(front), 삽입연산을 하는 곳을 리어(rear)라고 하며 식당에 들어가고 싶어서 줄이 길어지고 기다리다가 순서대로 식당에 들어가는 것을 생각하면 된다.
